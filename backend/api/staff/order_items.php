...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$orderId || !$serviceId || $qty <= 0) { echo json_encode(['success'=>false,'message'=>'Invalid input']); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    $itemId  = $_GET['item_id']  ?? '';
    if (!$orderId || !$itemId) { echo json_encode(['success'=>false,'message'=>'Invalid input']); exit; }

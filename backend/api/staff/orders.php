...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    $orderId = $_GET['id'] ?? '';
    if (!$orderId) { echo json_encode(['success'=>false,'message'=>'id required']); exit; }
    $ok = $staff->deleteOrder($orderId);
    echo json_encode(['success'=>$ok,'message'=>$ok?'Order deleted':'Delete failed']); exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
    ...
    $orderId = $input['id']     ?? '';
    $status  = $input['status'] ?? '';
    if (!$orderId || !$status) { echo json_encode(['success'=>false,'message'=>'id and status required']); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $items      = $input['items']       ?? [];
    if (!$customerId || empty($items)) { echo json_encode(['success'=>false,'message'=>'Customer and items required']); exit; }
    try {
